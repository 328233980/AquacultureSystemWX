...
public interface ReminderMapper {
    ...
    @Insert("INSERT INTO reminder (user_id, pond_id, reminder_type, title, content, remind_date, status, created_at, updated_at) " +
            "VALUES (#{userId}, #{pondId}, #{reminderType}, #{title}, #{content}, #{remindDate}, #{status}, datetime('now'), datetime('now'))")
    @Options(useGeneratedKeys = true, keyProperty = "id")
    int insert(Reminder reminder);
    
    @Update("UPDATE reminder SET status = #{status}, updated_at = datetime('now') WHERE id = #{id}")
